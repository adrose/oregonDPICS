...
for(d in id.rep){
  ...
  for(b in block.rep){
    ...
    if(dim(tmp.data)[1]>0){
      ...
      moreData <- TRUE
      z <- 1
      flagCheck <- nrow(tmp.data)
      while(moreData){
        print(z)
        row.prev <- z - 1
        currentState <- tmp.data$X4[z]
        ## Now idenitfy the next state that does not match
        matchVal <- TRUE
        nextVal <- z + 1
        countVal <- 1
        while(matchVal){
          nextState <- tmp.data$X4[nextVal]
          if(!is.na(nextState) & nextState != currentState){
            matchVal <- FALSE
            stayLength <- tmp.data$X3[nextVal] - tmp.data$X3[z]
          }else if(is.na(nextState)){
            matchVal <- FALSE
            stayLength <- 300 - tmp.data$X3[z]
            nextState <- sample(which(1:4 != currentState))[1]
          }
          else{
            nextVal <- nextVal + 1
            countVal <- countVal + 1
        ...
        ## Check flag for moreDat here
        z <- z + countVal
        if(z == flagCheck | z > flagCheck){
          moreData <- FALSE
        }
        out.row <- c(d,b, stayLength, currentState, nextState)
...
brms.dat$Block <- factor(brms.dat$X2,levels = c(1,2,3))
brms.dat$X5 <- as.numeric(brms.dat$X5)
brms.dat$stayLength <- as.numeric(brms.dat$X3)
brms.dat$currentState <- factor(brms.dat$X4, levels = c(1,2,3,4))
brms.dat$nextState <- factor(brms.dat$X5, levels = c(1,2,3,4))
brms.dat$subject <- strSplitMatrixReturn(brms.dat$X1, "_")[,1]
brms.dat$wave <- strSplitMatrixReturn(brms.dat$X1, "_")[,2]
...
for(i in unique(mod.dat2$X1)){
  for(b in 1:3){
    tmp.dat <- brms.dat[which(brms.dat$X1==i & brms.dat$Block==b),]
    if(dim(tmp.dat)[1]>0){
      est.mat <- statetable.msm(state = currentState, subject = X1, data = tmp.dat)
...
## Now intra transition rows
all.trans.counts <- all.trans.counts[-which(all.trans.counts$from == all.trans.counts$to),]
## Now attach the data 
all.trans.countsCAPS <- merge(all.trans.counts, caps.data, by.x=c("Subj"), by.y="FAMILY")
all.trans.countsCAPSMOD <- all.trans.countsCAPS %>% filter(Block == 3) %>% 
  filter(!is.na(Group))
modP <- brms::brm(value ~ (trans.pat + Wave + Group)^3 + (1|Subj), data = all.trans.countsCAPSMOD, family = "poisson",iter = 6000, warmup = 2000,cores = 5, chains = 5,seed=16, control = list(max_treedepth=15, adapt_delta=.9))
## Now plot some of thee interactions
plot.dat <- all.trans.countsCAPSMOD%>% filter(!is.na(Group))
plot.dat$predVals <- predict(modP)[,1]
plot.vals <- summarySE(plot.dat, measurevar = "predVals", groupvars = c("trans.pat", "Block", "Wave", "Group"))
plot.vals %>% ggplot(., aes(x=trans.pat, y=predVals, group=Wave, color=Wave, fill=Wave)) +

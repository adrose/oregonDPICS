...
## Now I want to organize the data so the events are compliance and the time until represents time until compliance is achieved
## In order to perform this -- I am going to loop through every individual in the data frame and every one of their compliance interactions
rm(tmp.dat, tmp.data)
all.compliance <- NULL
id.rep2 <- id.rep[-which(id.rep %in% c("CAPS108_3", "CAPS151_1", "CAPS168_3", "CAPS182_1", "CAPS187_1", "CAPS242_3", "CAPS320_1","CAPS123_1", "CAPS237_1", "CAPS405_1"))]
for(d in id.rep2){
  tmp.data <- brms.datmod[which(brms.datmod$V1 == d),]
  tmp.data$timePass <- cumsum(tmp.data$stayLength)
  total.compliance <- which(tmp.data$childBehavior=="Comply")
  all.compliance.SUBJ <- NULL
  for(c in 1:length(total.compliance)){
    ## Identity all values that occur between previous and current comply
    ## Add a flag if this is the first state
    prev.comply <- 1
    if(c == 1){
      prev.row.index <- 1
    }else{
      prev.comply <- c - 1
      prev.row.index <- total.compliance[prev.comply] + 1
    }
    current.row.index <- total.compliance[c]
    prev.vals <- tmp.data[prev.row.index:current.row.index,]
    ## Now prepare an output row detailing all time spent in states that occur before the current compliance
    output.vals <- data.frame(positive=as.numeric(0), neutral=as.numeric(0), dont=as.numeric(0), disengagaed=as.numeric(0))
    ## Now go through every action taken and add it to the output row
    for(z in 1:nrow(prev.vals)){
      ## Identify the states
      ## Use these for time in state
      # if(prev.vals[z,"currentState"]==1){output.vals$positive <- output.vals$positive + prev.vals$stayLength[z]}
      # if(prev.vals[z,"currentState"]==2){output.vals$neutral <- output.vals$neutral + prev.vals$stayLength[z]}
      # if(prev.vals[z,"currentState"]==3){output.vals$dont <- output.vals$dont + prev.vals$stayLength[z]}
      # if(prev.vals[z,"currentState"]==4){output.vals$disengagaed <- output.vals$disengagaed + prev.vals$stayLength[z]}
      ## Use these for counts of interactions
      if(prev.vals[z,"currentState"]==1){output.vals$positive <- output.vals$positive + 1}
      if(prev.vals[z,"currentState"]==2){output.vals$neutral <- output.vals$neutral + 1}
      if(prev.vals[z,"currentState"]==3){output.vals$dont <- output.vals$dont + 1}
      if(prev.vals[z,"currentState"]==4){output.vals$disengagaed <- output.vals$disengagaed + 1}
    }
    ## Now add the number of transitions
    output.vals$totalStates <- nrow(prev.vals)
    if(c == 1){
      output.vals$totalTime <- tmp.data$timePass[current.row.index]
      print(d)
    }else{
      output.vals$totalTime <- tmp.data$timePass[current.row.index] - tmp.data$timePass[prev.row.index]
    }
    all.compliance.SUBJ <- rbind(all.compliance.SUBJ, output.vals)
  }
  all.compliance.SUBJ$count <- 1:nrow(all.compliance.SUBJ)
  all.compliance.SUBJ$SUBJ <- d
  all.compliance <- rbind(all.compliance, all.compliance.SUBJ)
}
## Now fix the subject indicator
all.compliance$subject <- strSplitMatrixReturn(all.compliance$SUBJ, "_")[,1]
all.compliance$wave <- strSplitMatrixReturn(all.compliance$SUBJ, "_")[,2]
all.compliance$totalTime[which(all.compliance$totalTime==0)] <- all.compliance$totalTime[which(all.compliance$totalTime==0)] + runif(length(all.compliance$totalTime[which(all.compliance$totalTime==0)]), min = 0.5, max=5) 
## Reattach demo vals
all.complianceM <- merge(all.compliance, caps.data, by.x="subject", by.y="FAMILY")

## Now isolate data of interest
all.complianceM <- all.complianceM %>% filter(!is.na(Group)) %>% 
  filter(!is.na(CDINUM)) %>% 
  filter(totalTime < 115)

initial.brm <- brm(totalTime ~ (positive+neutral+dont+disengagaed+count+wave+CDINUM+PDINUM)+(1|subject), data = all.complianceM, family=weibull(),iter = 5000, warmup = 2000, cores = 1, chains = 1, control = list(max_treedepth=15, adapt_delta=.9), thin = 25, silent = 0)
#conditional_effects(initial.brm)

## Now train the compliance model here
all.compliance.scale <- all.complianceM
all.compliance.scale[,2:6] <- scale(all.compliance.scale[,2:6])[,]
all.compliance.scale$count <- scale(all.compliance.scale$count)
initial.brm2 <- brm(totalTime ~ (positive+neutral+dont+disengagaed+wave+CDINUM+PDINUM)^2+(1|subject), data = all.compliance.scale, family=weibull(),iter = 6000, warmup = 1500, cores = 1, chains = 1, control = list(max_treedepth=15, adapt_delta=.99), thin = 5)
#saveRDS(initial.brm2, file = "~/Documents/oregonDPICS/data/twoComplianceMod.RDS")
initial.brm3 <- brm(totalTime ~ (positive+neutral+dont+disengagaed+totalStates+wave+CDI_PDI_dose+count)^3+(1|subject), data = all.compliance.scale, family=weibull(),iter = 36000, warmup = 12000, cores = 8, chains = 8, control = list(max_treedepth=15, adapt_delta=.9), thin = 25)
saveRDS(initial.brm3, file = "~/Documents/oregonDPICS/data/twoComplianceMod.RDS")
saveRDS(initial.brm3, file = "~/Documents/oregonDPICS/data/thrComplianceMod.RDS")
initial.brm2 <- readRDS("~/Documents/oregonDPICS/data/twoComplianceMod.RDS")

## Now plot these points
all.compliance.scale$fitted <- fitted(initial.brm2)[,1]


## Now lets try to explore some three way interactions?
